...
__device__ void KeyExpansionCore(uint8_t* in, uint8_t i) {
...
__device__ void Round(uint8_t* state, uint8_t* key) {
    SubBytes(state);
    ShiftRows(state);
    MixColumns(state);
...
__device__ void FinalRound(uint8_t* state, uint8_t* key) {
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, key);
}
